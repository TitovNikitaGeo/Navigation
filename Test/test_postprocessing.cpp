...
int testPostprocessing::testSearchingNmea23WithOnboard()
{
    ...
    for (SegYReader::Pair pair: pr.pairs) {
        ...
        if (GGAendbuoy_first.dateTime.time().secsTo(pair.time) > 10) {
            // items[0]->hasConnection = false;
            qDebug() << items[0]->name << "connection false";
            qDebug() << pair.time;
        // } else {
            // items[0]->hasConnection = true;
            // qDebug() << items[0]->name << "connection true";
            // qDebug() << pair.time;
        }
        ...
        pr.p190->createP190File(pair.julianDay);
...
double testPostprocessing::getDistance(NmeaParser::NmeaGGAData prev, NmeaParser::NmeaGGAData next)
{
